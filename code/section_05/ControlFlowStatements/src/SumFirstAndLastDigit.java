/*
    ...
    number passed to the method, using a loop and return the sum of the first
    ...
    * sumFirstAndLastDigit(0); → should return 0, the first digit and the last
      digit is 0 since we only have 1 digit, which gives us 0+0 and the sum is
      0.
    * sumFirstAndLastDigit(5); → should return 10, the first digit and the last
      digit is 5 since we only have 1 digit, which gives us 5+5 and the sum is
      10.
...
public class SumFirstAndLastDigit {
    ...
    public static int sumFirstAndLastDigit (int number) {
        ...
        int firstNumber = number % 10; // First number is the most right number.
        int lastNumber = 0; // Last number is the most left number.

        while (number > 0) {
            // If number < 10, it means that it has only one digit.
            if (number < 10) {
