/*
    ...
    (5 kilos each).
    The parameter smallCount represents the count of small flour bags
    (1 kilo each).
    ...
    * canPack (0, 5, 4); should return true since smallCount is 5 (small bags of
      1 kilo) and goal is 4 kilos, and we have 1 bag left which is ok as
      mentioned above.
...
public class FlourPackProblem {
    ...
    public static boolean canPack (int bigCount, int smallCount, int goal) {
        ...
        int remainingPackageWeight = goal;
        int bigBagsUsed;
        int smallBagsUsed;
        int bigBagWeight = 5;
        int smallBagWeight = 1;

        while (bigCount + smallCount > 0) {
            bigBagsUsed = 0;
            smallBagsUsed = 0;

            if (bigCount > 0 && remainingPackageWeight >= bigBagWeight) {
                bigBagsUsed = Math.min(
                        remainingPackageWeight / bigBagWeight, bigCount);
                remainingPackageWeight -= bigBagsUsed * bigBagWeight;
                bigCount -= bigBagsUsed;
            }

            if (smallCount > 0 && remainingPackageWeight >= smallBagWeight) {
                smallBagsUsed = Math.min(
                        remainingPackageWeight / smallBagWeight, smallCount);
                remainingPackageWeight -= smallBagsUsed * smallBagWeight;
                smallCount -= smallBagsUsed;
            }

            if (remainingPackageWeight == 0) {
                return true;
            } else if (bigBagsUsed + smallBagsUsed == 0) {
                // This will check the use-case when we still haven't reached
                // the package goal, but we have bags, and they did not fit into
                // package.
                return false;
            }
        }
